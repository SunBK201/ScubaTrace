...
from .statement import RustBlockStatement, SimpleStatement
...
class RustFunction(Function, RustBlockStatement):
    ...
    def __find_next_nearest_stat(self, stat: Statement) -> Statement | None:
        ...
        if stat_type == "return_expression":
            return None
        ...
        if index == len(parent_statements) - 1:
            ...
            if stat.parent.node.type in language.RUST.loop_statements:
                return stat.parent
            else:
                return self.__find_next_nearest_stat(stat.parent)
        if (
            stat.parent.node.type == "if_expression"
            and parent_statements[index + 1].node.type == "if_expression"
        ):
            assert isinstance(stat.parent, BlockStatement)
                ...
                match type:
                    case "continue_expression":
                        ...
                        while (
                            ...
                            and loop_stat.node.type not in language.RUST.loop_statements
