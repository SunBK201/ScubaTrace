...
from .statement import SimpleStatement, SwiftBlockStatement
...
class SwiftFunction(Function, SwiftBlockStatement):
    ...
    def __find_next_nearest_stat(self, stat: Statement) -> Statement | None:
        ...
        assert stat.node.text is not None
        if (
            stat_type == "control_transfer_statement"
            and stat.node.text.decode().startswith("return")
        ):
            return None
        ...
        if index == len(parent_statements) - 1:
            ...
            if stat.parent.node.type in language.SWIFT.loop_statements:
                return stat.parent
            else:
        ...
        if (
            ...
            assert isinstance(stat.parent, BlockStatement)
            alter_stat = stat.parent.statements_by_type("if_statement")
            if len(alter_stat) == 0:
                return parent_statements[index + 1]
            else:
                assert isinstance(stat.parent, BlockStatement)
                ...
                match type:
                    case "control_transfer_statement":
                        ...
                        while (
                            ...
                            not in language.SWIFT.loop_statements
